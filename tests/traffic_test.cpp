...
static bool forbidden(const std::string& s) {
  ...
  for (const char* w : {"collision", "conflict", "safe", "clear", "tcas"})
...
static TrafficObserver observer_of(const J* o) {
...
static void run_rule_vector(const J& v) {
  ...
  for (const J& step : v.get("steps")->a) {
    ...
    CHECK((int)r.near_count == (int)e.num("near_count"), "near_count %d", r.near_count);
    CHECK((int)r.ground_count == (int)e.num("ground_count"), "ground_count %d", r.ground_count);
...
static void test_edges(void) {
  ...
  // Capacity: 20 emergencies + 1 pair warning -> 16 kept, the warning first.
  ...
  for (int i = 0; i < 20; i++) {
    ...
    ac[i] = mk_ac(hx, 10.0 + i * 0.01, 20.0, 3000, now - 1000);
    ac[i].squawk = 7700;
  }
  ac[20] = mk_ac("aaaaaa", 10.0, 20.001, 120, now - 1000);
  ...
  TrafficObserver ob = {10.0, 20.0, 0.0};     // only aaaaaa (120 m) is LOW
  traffic_evaluate(&dr, 1, ac, 21, &ob, true, now - 1000, now, &st, &r);
  CHECK(r.n == TRAFFIC_MAX_ALERTS, "capacity %d", r.n);
  CHECK(r.alerts[0].kind == TRAFFIC_KIND_NEAR && !strcmp(r.alerts[0].hex, "aaaaaa"), "warning kept first");
  CHECK(r.alerts[1].kind == TRAFFIC_KIND_LOW && !strcmp(r.alerts[1].hex, "aaaaaa"), "caution second");
  // then the nearest emergencies: e00000 .. e00013
  bool nearest = true;
  for (int i = 2; i < r.n; i++) { char hx[8]; snprintf(hx, sizeof(hx), "e%05d", i - 2); nearest &= !strcmp(r.alerts[i].hex, hx); }
  CHECK(nearest, "the 14 nearest emergencies kept");
  ...
  CHECK(!strcmp(s, "no ADS-B source"), "summary %s", s);
...
int main(int argc, char** argv) {
  ...
  g_ctx = "totals";
  CHECK(files.size() >= 13, "vector files %zu", files.size());
  CHECK(rule_files >= 12 && g_steps >= 36 && g_alerts >= 60 && g_cases >= 8,
